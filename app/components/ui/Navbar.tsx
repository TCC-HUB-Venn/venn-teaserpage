...
export const Navbar = () => {
  ...
  return (
    <>
      {/* ===== Navbar Desktop ===== */}
      <header className='hidden md:flex items-center justify-center  rounded-full border border-white/10 bg-[#0A0A0A]/55 py-4 px-8 w-fit mx-auto mt-4 fixed top-0 left-0 right-0 backdrop-blur-3xl z-50 shadow-[0_0_30px_6px_rgba(37,99,235,0.10)] gap-16'>
        <Image src='/assets/logo.png' alt='Logo' width={120} height={120} className='' />
        <nav>
                ...
                <li key={section.id}>
                  <Link
                    ...
                    className={`rounded-full px-4 py-3 transition-colors ${
                      isActive ? 'bg-primary text-white' : 'text-white/80 hover:text-white'
                    }`}
                  >
                    {section.label}
                  </Link>
                  ...
                  >
                    <span
                      className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${
                        isActive ? "bg-primary scale-100" : "bg-white/20 scale-75 group-hover:bg-white/40"
                      }`}
                    />
                    {section.label}
                    {isActive && (
